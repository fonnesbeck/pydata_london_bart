...
def _(mo):
    ...
    **Convergence diagnostics.** Two numbers tell you whether the chains
    ...
    the practical target). The application notebooks 02 through 04 plot both
...
def _(mo):
    ...
    **In the application notebooks.** `pymc-bart` ships `pmb.plot_pdp` and
    ...
    a `pmb.BART` random variable. Notebook 02 uses `plot_pdp` for the
    Friedman headline fit, and notebook 04 uses `plot_ice` because in
    survival analysis the per-individual hazard curve is what stakeholders
