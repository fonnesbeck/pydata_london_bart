...
The tutorial is delivered as one slide-source notebook, one PyMC introduction
notebook, plus two hands-on application notebooks:
...
- **`classification.py` — BART for classification (15 min, hands-on):** Predict whether GSS respondents report very high life satisfaction, check held-out probabilities and reliability, compare against logistic regression, then return to the full 1–10 ladder with an ordered-probit BART model.
...
pixi run marimo check pymc_intro.py regression.py classification.py slides/how_bart_works.py  # structural validation
```
